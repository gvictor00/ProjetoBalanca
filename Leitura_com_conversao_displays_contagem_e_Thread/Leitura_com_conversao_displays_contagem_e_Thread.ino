...
char commandByte = 0;
int serialCounter = 0; // Quantidade de itens no buffer
...
// Roda de forma concorrente com o programa
void sensorUltra() {
...
void setup() {
  ...
  // Threads
  dist.onRun(sensorUltra);
  dist.setInterval(200); 
...
void loop() {
  // Thread para cálculo da distância. Ela roda a cada 200ms
  if (dist.shouldRun())
    dist.run();
